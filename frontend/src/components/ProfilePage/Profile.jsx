...
const Profile = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL}journal/getUserJournal`,
        {
          headers: {
  ...
  const handleDelete = async (userName, title) => {
    const token = getCookieValue("journal_token");
    try {
      const deletion = await axios.post(
      ...
      <div className=" flex flex-col gap-x-5 my-5 rounded-xl">
        <div className="rounded-xl px-12">
          ...
          <div className="mt-10">
            ...
            <p className="text-5xl shadow-lg py-4 rounded-xl bg-green-400 text-center">
              Your Articles
            </p>
            ...
            <div className="mt-10 rounded-xl">
              {data.map((journal, index) => (
                <div
                  ...
                  } sm:p-4 my-[1rem] shadow-md text-[#333]`}
                >
                  <img
                    src={journal.image}
                    alt="journal"
                    className="h-[300px] rounded-lg hidden lg:block"
                  />
                  <div className="sm:px-6 h-[20rem] w-full">
                    ...
                    >
                      <button
                        className="p-[0.7rem] text-2xl bg-green-400 rounded-md"
                        onClick={() => {
                      ...
                      <button className="p-[0.7rem] text-2xl bg-red-500 rounded-md">
                        <MdDeleteOutline
                          onClick={() => {
                            handleDelete(journal.userName, journal.title);
                          }}
                        />
                      </button>
                    </div>
